...
export const ACHIEVEMENTS = [
  {
    ...
    icon: "🏁",
    desc: "Registrou a primeira sessão",
  },
  {
    code: "streak_7",
    name: "7 dias seguidos",
    icon: "🔥",
    desc: "Atividade em 7 dias consecutivos",
  },
  {
    code: "streak_30",
    name: "30 dias seguidos",
    icon: "🌋",
    desc: "Atividade em 30 dias consecutivos",
  },
...
export async function checkAchievements(): Promise<(typeof ACHIEVEMENTS)[number][]> {
  ...
  if (fresh.length) {
    ...
    // grava a lista completa (lida do banco), nunca um updater sobre cache
    // possivelmente frio — evita perder conquistas antigas
    const full: UnlockedAchievement[] = [
      ...unlocked,
      ...fresh.map((f) => ({ code: f.code, date: now })),
    ];
    writeStore<UnlockedAchievement[]>(KEYS.achievements, full, []);
  }
  return fresh;
}
