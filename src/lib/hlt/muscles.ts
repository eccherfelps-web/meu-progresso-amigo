...
export const MUSCLES = [
  ...
  "Quadríceps", "Posterior de Coxa", "Panturrilha", "Abdômen",
] as const;
...
export interface MuscleStat { muscle: Muscle; sets: number; volume: number; pct: number }
...
export function weeklyMuscleStats(sessions: WorkoutSession[], days = 7): MuscleStat[] {
  ...
  return order
    ...
    .map((m) => {
      const v = acc.get(m)!;
      return { muscle: m, sets: v.sets, volume: Math.round(v.volume), pct: Math.round((v.volume / total) * 100) };
    });
}
...
export interface BalanceReport { score: number; pairs: BalancePair[] }
...
export function muscleBalance(stats: MuscleStat[]): BalanceReport {
  ...
  const defs: { label: string; a: [string, number]; b: [string, number]; ideal: [number, number]; low: string; high: string }[] = [
    { label: "Peito × Costas", a: ["Peito", vol("Peito")], b: ["Costas", vol("Costas")], ideal: [0.8, 1.25],
    ...
    { label: "Quadríceps × Posterior", a: ["Quadríceps", vol("Quadríceps")], b: ["Posterior", vol("Posterior de Coxa")], ideal: [0.9, 1.7],
    ...
    { label: "Bíceps × Tríceps", a: ["Bíceps", vol("Bíceps")], b: ["Tríceps", vol("Tríceps")], ideal: [0.6, 1.4],
      low: "Tríceps dominando — 2 séries extras de rosca resolvem.",
      high: "Bíceps dominando — adicione tríceps corda/francês." },
    { label: "Empurrar × Puxar", a: ["Empurrar", sum("Peito", "Ombros", "Tríceps")], b: ["Puxar", sum("Costas", "Bíceps", "Antebraço")], ideal: [0.8, 1.3],
  ...
  let penalty = 0, evaluated = 0;
  for (const d of defs) {
    const [an, av] = d.a, [bn, bv] = d.b;
    if (av === 0 && bv === 0) continue; // sem dados deste par na semana
    let ratio: number | null = null, ok = true, suggestion: string | null = null;
    if (av === 0 || bv === 0) {
      ok = false;
      suggestion = av === 0 ? `Nenhum volume de ${an} esta semana — ${d.low}` : `Nenhum volume de ${bn} esta semana — ${d.high}`;
      penalty += 25;
    } else {
    ...
    evaluated++;
    pairs.push({ label: d.label, a: { name: an, volume: av }, b: { name: bn, volume: bv }, ratio, ideal: d.ideal, ok, suggestion });
  }
