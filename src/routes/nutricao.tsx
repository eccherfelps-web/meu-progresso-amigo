...
import { searchFoodsOnline } from "@/lib/hlt/foodApi";
import { TACO_FOODS } from "@/lib/hlt/tacoFoods";
...
function NutricaoPage() {
  ...
  const totals = useMemo(() => {
    ...
    for (const m of MEALS)
      for (const i of todayLog.meals[m.key]) {
        t.kcal += i.kcal;
        t.p += i.protein_g;
        t.c += i.carbs_g;
        t.f += i.fat_g;
  ...
  const addItem = (meal: MealKey, item: FoodItem) => {
    setFoods((prev) => {
      ...
      if (idx === -1)
        return [
          ...prev,
          { ...emptyLog(today), meals: { ...emptyLog(today).meals, [meal]: [item] } },
        ];
      const updated = [...prev];
      updated[idx] = {
        ...updated[idx],
        meals: { ...updated[idx].meals, [meal]: [...updated[idx].meals[meal], item] },
      };
      return updated;
    });
  ...
  const removeItem = (meal: MealKey, i: number) => {
    setFoods((prev) =>
      prev.map((f) =>
        f.date === today
          ? { ...f, meals: { ...f.meals, [meal]: f.meals[meal].filter((_, j) => j !== i) } }
          : f,
      ),
    );
  };
  ...
  const adjustCups = (delta: number) => {
    setHydration((prev) => {
      ...
      const u = [...prev];
      u[idx] = { ...u[idx], cups: Math.max(0, u[idx].cups + delta) };
      return u;
    });
  };
    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto">
      ...
      <Card className="mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                {cups} copos · {((cups * 250) / 1000).toFixed(2)}L /{" "}
                {(waterGoalMl / 1000).toFixed(2)}L
              </div>
            </div>
          </div>
          <div className="flex gap-1">
            <Button size="sm" variant="outline" onClick={() => adjustCups(-1)}>
              −
            </Button>
            <Button size="sm" onClick={() => adjustCups(1)}>
        ...
        <div className="h-2 rounded-full bg-muted overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, (cups * 250 * 100) / waterGoalMl)}%` }}
          />
        </div>
      </Card>
      ...
      <div className="space-y-4">
        {MEALS.map((m) => {
          ...
          const sub = items.reduce((a, i) => ({ kcal: a.kcal + i.kcal, p: a.p + i.protein_g }), {
            kcal: 0,
            p: 0,
          });
          return (
            <Card key={m.key}>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="text-xs text-muted-foreground">
                  {Math.round(sub.kcal)} kcal · {Math.round(sub.p)}g prot.
                </div>
              </div>
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="truncate">
                        {i.name} <span className="text-muted-foreground text-xs">({i.grams}g)</span>
                      </div>
                      <div className="text-xs text-muted-foreground">
                        {Math.round(i.kcal)} kcal · P {i.protein_g.toFixed(1)} · C{" "}
...
function AddFoodDialog({ onAdd }: { onAdd: (i: FoodItem) => void }) {
  ...
  // banco local = base original + tabela TACO (alimentos brasileiros), sem duplicatas
  const LOCAL_DB = useMemo(() => {
    const seen = new Set(FOOD_DB.map((f) => f.name.toLowerCase()));
    return [...FOOD_DB, ...TACO_FOODS.filter((f) => !seen.has(f.name.toLowerCase()))];
  }, []);
  const filtered = LOCAL_DB.filter((f) =>
  ...
  const searchOnline = async () => {
    ...
    setOnline([]);
    try {
      const { foods, fromCache } = await searchFoodsOnline(search.trim());
      setOnline(foods);
      if (!foods.length)
        setOnlineMsg(
          `Nenhum resultado online para "${search.trim()}" — tente outro termo ou cadastre manualmente abaixo.`,
        );
      else if (fromCache) setOnlineMsg("⚡ Resultado do cache local (busca recente).");
    } catch (err) {
      setOnlineMsg(
        err instanceof Error ? err.message : "Busca online indisponível — verifique a conexão.",
      );
    } finally {
      setSearching(false);
    }
  ...
  const pickDb = (f: (typeof FOOD_DB)[0]) => {
    ...
    onAdd({
      ...
      grams: g,
      kcal: f.kcal * factor,
      protein_g: f.protein_g * factor,
      carbs_g: f.carbs_g * factor,
      fat_g: f.fat_g * factor,
    });
    setOpen(false);
    setSearch("");
  };
  ...
  const addCustom = () => {
    ...
    setOpen(false);
    setCustom({ name: "", kcal: "", protein_g: "", carbs_g: "", fat_g: "" });
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" className="w-full">
          <Plus className="size-4 mr-1" /> Adicionar alimento
        </Button>
      </DialogTrigger>
      <DialogContent className="max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          ...
          <div className="max-h-60 overflow-y-auto border border-border rounded-md">
            {filtered.map((f) => (
              <button
                key={f.name}
                onClick={() => pickDb(f)}
                className="block w-full text-left px-3 py-2 hover:bg-accent border-b border-border last:border-0"
              >
                <div className="text-sm font-medium">{f.name}</div>
                <div className="text-xs text-muted-foreground">
                  {f.kcal} kcal/100g · P {f.protein_g} · C {f.carbs_g} · G {f.fat_g}
                </div>
              </button>
            ))}
            {filtered.length === 0 && (
          ...
          {onlineMsg && (
            <div className="text-xs text-muted-foreground text-center">{onlineMsg}</div>
          )}
          {online.length > 0 && (
              ...
              {online.map((f, i) => (
                ...
                >
                  <div className="text-sm font-medium flex items-center gap-1.5">
                    <Globe className="size-3 text-info shrink-0" />
